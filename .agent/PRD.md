...
### 10.2. Dreams API (꿈 해몽 및 데이터)

| ID       | Method | Endpoint               | Scenario                                        | Expected Status  | Implementation Status |
| :------- | :----- | :--------------------- | :---------------------------------------------- | :--------------- | :-------------------- |
| API-D-01 | POST   | `/api/dreams`          | 꿈 데이터 생성 및 해몽 요청 (정상)              | 201 Created      | ✅ Pass               |
| API-D-02 | POST   | `/api/dreams`          | 필수 파라미터(content 등) 누락 시 에러          | 400 Bad Request  | ⬜️ Pending            |
| API-D-03 | GET    | `/api/dreams`          | 피드 목록 조회 (`?type=feed`)                   | 200 OK           | ⬜️ Pending            |
| API-D-04 | GET    | `/api/dreams`          | 내 꿈 목록 조회 (`?type=my`, 인증됨)            | 200 OK           | ⬜️ Pending            |
| API-D-05 | GET    | `/api/dreams`          | 권한 없는 상태에서 내 꿈 목록 조회 시도         | 401 Unauthorized | ⬜️ Pending            |
| API-D-06 | GET    | `/api/dreams/[id]`     | 개별 꿈 상세 조회 성공                          | 200 OK           | ⬜️ Pending            |
| API-D-07 | GET    | `/api/dreams/[id]`     | 존재하지 않는 꿈 조회 시도                      | 404 Not Found    | ⬜️ Pending            |
| API-D-08 | PATCH  | `/api/dreams/[id]`     | 꿈 상태 수정 성공 (공개 여부 변경 등)           | 200 OK           | ⬜️ Pending            |
| API-D-09 | PATCH  | `/api/dreams/[id]`     | 권한이 없는 꿈 무단 수정 시도                   | 403 Forbidden    | ⬜️ Pending            |
| API-D-10 | DELETE | `/api/dreams/[id]`     | 개별 꿈 데이터 삭제 성공                        | 200 OK           | ⬜️ Pending            |
| API-D-11 | DELETE | `/api/dreams/[id]`     | 권한이 없는 꿈 무단 삭제 시도                   | 403 Forbidden    | ⬜️ Pending            |
| API-D-12 | POST   | `/api/dreams/generate` | 내부망(Internal) 결제 완료 후 AI 생성 요청 처리 | 200 OK           | ✅ Pass               |

### 10.3. Payments API (결제 처리)
...
| API-P-07 | POST   | `/api/webhooks/toss`      | 인증되지 않은 IP/소스의 웹훅 무시          | 403 / 401       | ⚠️ Partial (TODO)     |
