...
|                             | 주문 상세 (`/.../[id]`)             | - 주문 기본 정보 섹션: 회원/비회원 정보, 결제 상태, 금액.<br>- 꿈 상세 데이터 섹션: 사용자 입력 텍스트와 AI 출력 텍스트, 생성된 이미지 전시.<br>- **(CS용 액션)**: Re-try (AI 해몽 재시도) 버튼, **Image Re-gen (이미지 재생성)** 버튼, Payment Cancel (결제 취소/환불) 버튼 액션 및 결과 모달/토스트 UI. | `GET /api/admin/orders/[id]`<br>`POST /api/admin/orders/[id]/retry`<br>`POST /api/admin/orders/[id]/image-regen`<br>`POST /api/admin/orders/[id]/cancel` | ✅ 완료   | ✅ 완료    |
...
| Method     | Endpoint               | Description                                                              | Access          |
| :--------- | :--------------------- | :----------------------------------------------------------------------- | :-------------- |
| **POST**   | `/api/dreams`          | 꿈 데이터 생성 및 해몽 요청 (Pending 상태 생성, `has_image_gen` 포함)    | User, Guest     |
| **POST**   | `/api/dreams/generate` | 내부망(Internal) 통신용 AI 분석 및 **이미지 생성(필요 시)** 진행 및 저장 | Internal Server |
...
| `image_url`       | `text`        | Null   | 생성된 이미지 URL (Imagen 등 AI 생성 이미지) |
...
| API-D-15 | POST   | `/api/dreams/generate` | 이미지 포함 옵션 구매 시 실제 이미지 생성 성공 및 URL 저장 검증 | 200 OK           | ✅ Pass               |
| API-D-16 | POST   | `/api/dreams/generate` | 이미지 생성 실패 시 예외 처리 및 관리자 알림 검증               | 200 OK           | ✅ Pass               |
...
| API-AD-02 | GET    | `/api/admin/orders`                 | 전체 결제/주문 리스트 목록 조회 (startDate, endDate 필터 지원)            | 200 OK          | ✅ Pass               |
| API-AD-03 | GET    | `/api/admin/orders/[id]`            | 특정 주문 및 연관 꿈 상세 정보 조회                                       | 200 OK          | ✅ Pass               |
| API-AD-04 | POST   | `/api/admin/orders/[id]/retry`      | 실패/대기 중인 꿈 해몽 재요청 (텍스트+이미지 전체)                        | 200 OK          | ✅ Pass               |
| API-AD-05 | POST   | `/api/admin/orders/[id]/cancel`     | 완료된 주문에 대해 토스 결제 취소요청                                     | 200 OK          | ✅ Pass               |
| API-AD-06 | GET    | `/api/admin/users`                  | 전체 회원(User) 및 비회원(Guest) 목록 조회 (startDate, endDate 필터 지원) | 200 OK          | ✅ Pass               |
| API-AD-07 | GET    | `/api/admin/dreams/public`          | 퍼블릭으로 설정된 전체 피드 조회 (startDate, endDate 필터 지원)           | 200 OK          | ✅ Pass               |
...
| API-AD-10 | POST   | `/api/admin/orders/[id]/retry`      | force 파라미터를 통한 이미 완료된 주문의 이미지 재생성                    | 200 OK          | ✅ Pass               |
...
| **SEO**       | 동적 Meta/OG 태그 삽입          | 개별 꿈 결과 페이지(`/dream-result/[id]`) 소스코드 조회 시 동적 title 및 og:image 태그가 정상적으로 치환되었는지 검증 | ✅ Pass   |
| **UX/AI**     | Streaming Response 확인         | 결제 완료 직후 AI 프롬프트가 실행될 때 브라우저가 행(Hang)에 걸리지 않고 텍스트를 스트리밍해 주는지 확인              | ✅ Pass   |
| **UX/AI**     | 중복 제출 방지 (디바운싱)       | 결제 폼 또는 프로필 저장 버튼 더블클릭/다중클릭 시 API 다중 호출이 무시되고 단일 호출만 전달되는지 검증               | ✅ Pass   |

## 13. Compliance Checklist (결제 및 개인정보 규제 준수)

AI Dream Teller 서비스가 사용자의 결제 정보와 개인적인 꿈 이야기, 개인 식별 정보(PII)를 다루는 만큼, 보안을 넘어선 **국제/국내 컴플라이언스(법적 규제 준수)**를 MECE 관점에서 엄격히 관리해야 합니다.

### 13.1 Privacy Data Protection (개인정보 보호)

- **수집의 최소화 확인**: 회원가입 시 이메일, 닉네임 외의 필수적이지 않은 정보(주소, 주민등록번호 등)는 수집하지 않도록 정책 수립.
- **명시적 동의 창구 (Opt-in Consent)**:
  - 개인정보 처리방침 및 서비스 이용약관 동의 팝업/체크박스가 명확히 분리되어 있는지 검증.
  - 마케팅 활용 동의(선택)를 필수 동의와 강제로 결합하지 않았는지 확인.
- **파기 및 잊혀질 권리 보장**:
  - 회원이 탈퇴(`DELETE /api/users/me`)를 요청할 시 지체 없이 관련 User Profile 및 Dreams DB Raw 데이터가 하드 딜리트(Hard Delete) 되는지 확인.
  - 장기 미접속 사용자(1년 이상)의 계정을 휴면 정책으로 전환하고 분리 보관하는 구조 점검.
- **비회원 30일 보관 정책**: 약관에 명시된 정책에 따라, 비회원 데이터(`guest_id` 종속)가 30일 경과 후 자동 파기되도록 DB 스케줄러(pg_cron 등)가 올바르게 작동하는지 확인.

### 13.2 Payment processing Compliance (결제 처리 규제 점검)

- **PG사(Toss Payments) 가이드라인 준수**:
  - 신용카드사 및 금감원의 지침에 위배되지 않는 결제 흐름인지 재검증 (e.g., 과도한 Active X 요구나 브라우저 차단 우회 요청 없음).
  - **시크릿 키(Secret Key) 보안 통제**: 결제 위젯 발급용 시크릿 키는 반드시 서버 환경(`.env` 등)에서 환경변수로 은닉해야 하며, GitHub 빌드 로그나 레포지토리에 하드 코딩되어 노출되지 않도록 Gitleaks 등의 스캐닝 체계와 `Basic Base64` 인증 로직을 서버에서만 구성함.
  - **환금성 상품 및 비인증 결제 심사 우회 금지**: 제공하는 서비스가 비인증 결제나 환금성이 높은 금융성 상품이 아닌 '결제 즉시 소모되는 순수 디지털 컨텐츠'임을 카드사 심사(Tosspayments) 자료로 명확히 소명해야 함.
- **결제 기록 보존**:
  - 전자상거래 등에서의 소비자보호에 관한 법률 등 관련 법령에 의거하여 **대금 결제 및 재화 등의 공급에 관한 기록은 5년간 필수 보존**해야 함. (사용자가 탈퇴하더라도 탈퇴 처리는 하되 결제 식별 거래 기록(Order Table) 내역은 암호화된 채로 5년 보존 정책 적용 필요.)
- **디지털 컨텐츠 청약 철회 규정**: 사용자가 결제 후 직접 AI 해몽 생성을 완료(소비)한 시점부터는 '디지털 콘텐츠(서비스)의 제공이 개시된 경우'로 간주하여 이용약관 내 전자상거래법 제 17조 2항에 따른 반환/환불 불가 원칙을 렌더링하고 동의를 받아야 함.
- **PCI-DSS (Payment Card Industry Data Security Standard) 준수 면제 확인**: 서비스 측 데이터베이스(Supabase)에 고객의 신용카드 번호, CVC, 비밀번호 등을 직접 평문 또는 암호화 저장하지 않음으로써, 심각한 유출 리스크를 원천 차단.

### 13.3 AI & Data Ownership (AI 활용 및 데이터 저작권)

- **AI 학습 사용 동의**: 사용자가 제출한 꿈 내용이나 해몽 결과가 타 AI 모델의 학습 데이터(Training Data)로 제공되지 않음을 "프라이버시 정책"에 단단히 명시.
- **지적 재산권 분쟁 방지 (Generated Content License)**: AI를 통해 생성된 이미지 및 해석 텍스트에 대한 소유권을 명확히 규정하여 약관에 병기 (예: 개인 용도의 상업적 재사용 가능 여부 등 구글 Gen AI 정책과 충돌 확인).

### 13.4 Compliance E2E Test Checklist

| 카테고리    | 점검 항목            | 주요 점검 내용 (규약 기반 시나리오)                                                                                                 | 상태    |
| :---------- | :------------------- | :---------------------------------------------------------------------------------------------------------------------------------- | :------ |
| **Privacy** | 강제 동의 방지       | 결제 진행 전 필수 약관 외 '선택 동의'를 제외하고도 정상 결제가 진행/렌더링 되는 흐름인지 확인                                       | ✅ Pass |
| **Privacy** | 잊혀질 권리 이행     | 이용자 '회원 탈퇴' API 호출(`DELETE /api/users/me`) 시 정상 파기 처리(200 OK)가 구동되는지 검증                                     | ✅ Pass |
| **Payment** | Secret Key 노출 방지 | Network Traffic 상의 모든 페이지 응답 및 API 페이로드에 토스페이먼츠 시크릿 키(`test_sk_...`) 평문이 절대 유출되지 않는지 확인      | ✅ Pass |
| **Payment** | 청약 철회 불가 고지  | 결제 UI 혹은 유의사항 화면 요소에 "결제 및 해몽 컨텐츠 제공 개시 시 환불 불가" 관련 텍스트가 물리적으로 명시/안내되고 있는지 검수   | ✅ Pass |
| **Payment** | 결제 영수증 보존     | 탈퇴 처리 후에도, 관리자 권한 API(`/api/admin/orders`)를 통해 탈퇴자의 이전 결제 거래 기록(Transaction)은 유실 없이 응답되는지 확인 | ✅ Pass |
| **AI/Data** | AI 자산/사용 고지    | 서비스 메인 페이지의 안내 영역에 "AI 학습 미사용" 및 "AI 생성물 저작권/환불"에 대한 고지 텍스트가 존재하는지 점검                   | ✅ Pass |
